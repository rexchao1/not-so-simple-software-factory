...
CREATE TABLE automation_schedule_triggers (
...
CREATE TABLE automation_schedule_occurrences (
    ...
    timezone TEXT NOT NULL,
    CHECK (
        (kind = 'scheduled' AND scheduled_at IS NOT NULL AND run_request_key IS NULL)
        OR
        (kind = 'run_now' AND scheduled_at IS NULL AND run_request_key IS NOT NULL)
    ),
    UNIQUE (automation_id, scheduled_at),
    UNIQUE (automation_id, run_request_key)
);
...
ON automation_schedule_triggers(next_due_at, automation_id);

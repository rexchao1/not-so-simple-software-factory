...
// Planning state moved into the database, so roadmap_root names nothing any
// more. An unknown field is a hard error here, so the field stays decoded and
// is ignored: refusing to boot over a setting that is merely obsolete would
// turn an upgrade into an outage on a machine nobody works on directly.
func TestServerBootstrapConfigIgnoresRetiredRoadmapRoot(t *testing.T) {
	dataRoot := t.TempDir()
	path := filepath.Join(dataRoot, "config.toml")
	t.Setenv("FACTORY_SERVER_CONFIG", path)
	if err := os.WriteFile(path, []byte(`
listen = "127.0.0.1:7337"
roadmap_root = "/Users/someone/Projects/chao-orchestrator/state"
`), 0o600); err != nil {
		t.Fatal(err)
	}
	config, err := loadServerBootstrapConfig(dataRoot)
	if err != nil {
		t.Fatalf("a config naming roadmap_root must still load: %v", err)
	}
	if config.Listen != "127.0.0.1:7337" {
		t.Fatalf("loaded config = %#v", config)
	}
	// The value is kept only so startup can name it in the one warning it
	// logs. Nothing reads it as a path, so it is not resolved against the
	// config file's directory the way a live path setting would be.
	if config.RetiredRoadmapRoot != "/Users/someone/Projects/chao-orchestrator/state" {
		t.Errorf("retired roadmap root = %q, want the raw configured value", config.RetiredRoadmapRoot)
	}
}

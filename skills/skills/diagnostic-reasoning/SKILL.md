...
description: >-
  Procedure for diagnosing reported bugs.
...
It applies both when investigating directly and when briefing a delegated investigation and judging the evidence that comes back.
...
If a load-bearing element is missing, run a focused follow-up investigation instead of treating confidence or implementation detail as proof.
A diagnosis or implementation-ready recommendation is evidence, not authorization to change code.
Implementation still requires the user's request or an existing authority in the workflow that runs this agent, and the reproduction should become the regression test when a fix is authorized.
